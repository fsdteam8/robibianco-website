...
import type { ReviewData, SubmitReviewResponse } from "@/types"
...
export default function ReviewForm({ onSubmit }: ReviewFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://localhost:5001/api/v1/spin-win/submit-review", {
        method: "POST",
        headers: {
      ...
      const result: SubmitReviewResponse = await response.json()

      if (result.success && result.data.reviewId) {
