...
export default function ResultScreen({ result, onBackToHome }: ResultScreenProps) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      onBackToHome()
    }, 30000) // 30 seconds auto-redirect
...
${result.prize.couponCode ? `Coupon Code: ${result.prize.couponCode}` : ""}
${result.prize.prizeCode ? `Prize Code: ${result.prize.prizeCode}` : `Prize Code: PRIZE-${Date.now()}`}
...
Thank you for spinning with us!
  ...
  const downloadQRCode = () => {
    ...
    link.download = `prize-qr-code-${result.prize.rewardName.replace(/\s+/g, "-").toLowerCase()}.png`
    link.href = qrDataUrl
  ...
  return (
    <div className="min-h-screen bg-[#48a256] relative overflow-hidden">
      ...
      <div className="relative z-10 px-4 py-8 sm:py-12 lg:py-16 text-center text-white">
        ...
        <p className="text-sm sm:text-base text-white/80 mt-6 sm:mt-8">
