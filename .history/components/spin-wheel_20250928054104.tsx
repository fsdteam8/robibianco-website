    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-sm sm:text-base">Loading wheel segments...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center px-4">
          <p className="text-red-600 mb-4 text-sm sm:text-base">Failed to load wheel segments</p>
          <button
            onClick={() => window.location.reload()}
            className="bg-[#f97316] text-white px-4 py-2 rounded-lg hover:bg-[#ea580c] text-sm sm:text-base"
          >
            Retry
          </button>
        </div>
    ...
    <div className="min-h-screen bg-white relative overflow-hidden">
      <div className="bg-[#48a256] text-white text-center py-6 sm:py-8 lg:py-12 relative">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-8 sm:h-12 lg:h-16">
          <svg viewBox="0 0 1200 120" className="w-full h-full" preserveAspectRatio="none">
            ...
            <path d="M0,0 C300,120 900,120 1200,0 L1200,120 L0,120 Z" fill="url(#curveGradient)" />
          </svg>
        </div>
      </div>
      ...
      <div className="px-4 py-6 sm:py-8 lg:py-12">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-700 mb-8 sm:mb-12 lg:mb-16 leading-relaxed px-4">
          ...
          <div className="relative inline-block mb-8 sm:mb-12 lg:mb-16">
            ...
            <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1 sm:-translate-y-2 z-20">
              <div className="w-0 h-0 border-l-3 border-r-3 border-b-6 sm:border-l-4 sm:border-r-4 sm:border-b-8 lg:border-l-6 lg:border-r-6 lg:border-b-12 border-l-transparent border-r-transparent border-b-gray-800"></div>
            </div>
            ...
            <div className="relative">
              <div
                ref={wheelRef}
                className="w-64 h-64 sm:w-80 sm:h-80 md:w-96 md:h-96 lg:w-[28rem] lg:h-[28rem] xl:w-[32rem] xl:h-[32rem] rounded-full relative overflow-hidden border-4 lg:border-6 border-gray-800 cursor-pointer transition-transform hover:scale-105"
                onClick={handleSpin}
              >
                <svg width="100%" height="100%" viewBox="0 0 400 400" className="transform -rotate-90">
                  ...
                  {wheelSegments.map((segment, index) => {
                    ...
                    const iconX = 200 + 80 * Math.cos(textAngle)
                    const iconY = 200 + 80 * Math.sin(textAngle)
                    ...
                    return (
                      <g key={segment.reward._id}>
                        ...
                        >
                          {segment.label.split("\n").map((line, lineIndex) => (
                            <tspan key={lineIndex} x={textX} dy={lineIndex === 0 ? 0 : "1.2em"}>
                              {line}
                            </tspan>
                          ))}
                        </text>
                        <text
                          ...
                          textAnchor="middle"
                          dominantBaseline="middle"
                          transform={`rotate(${textAngle * (180 / Math.PI) + 90} ${iconX} ${iconY})`}
                          className="select-none"
                        >
                ...
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-gray-800 rounded-full flex items-center justify-center z-10 shadow-lg">
                  <span className="text-white font-bold text-xs sm:text-sm lg:text-base">Spin</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            className={`w-full max-w-sm sm:max-w-md lg:max-w-lg mx-auto font-bold py-3 sm:py-4 lg:py-5 px-6 sm:px-8 lg:px-10 rounded-xl text-base sm:text-lg lg:text-xl transition-all duration-200 ${
              spinWheelMutation.isPending || hasSpun || wheelSegments.length === 0
                ? "bg-gray-400 cursor-not-allowed text-gray-600"
                : "bg-[#f97316] hover:bg-[#ea580c] text-white hover:scale-105 shadow-lg hover:shadow-xl"
            }`}
          >
          ...
          {spinWheelMutation.isPending && (
            <p className="text-gray-600 mt-4 animate-pulse text-sm sm:text-base lg:text-lg">
