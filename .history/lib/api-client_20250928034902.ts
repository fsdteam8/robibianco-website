...
class ApiClient {
  ...
  private setupInterceptors() {
    ...
    this.instance.interceptors.response.use(
      ...
      (error) => {
        console.error("[v0] API Response Error:", error.response?.status, error.response?.data)

        // Handle common error scenarios
        if (error.response?.status === 401) {
          // Handle unauthorized access
          console.error("[v0] Unauthorized access - consider redirecting to login")
        } else if (error.response?.status >= 500) {
          // Handle server errors
          console.error("[v0] Server error - consider showing user-friendly message")
        }
  ...
  updateBaseURL(newBaseURL: string) {
    ...
    console.log(`[v0] API Base URL updated to: ${newBaseURL}`)
  }
}
