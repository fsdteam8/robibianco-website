...
export function useSubmitReview() {
  return useMutation<SubmitReviewResponse, Error, ReviewData>({
    ...
    onError: (error) => {
      console.error("[v0] Submit review mutation error:", error)
    },
  })
}
...
export function useSpinWheel() {
  return useMutation<SpinWheelResponse, Error, string>({
    mutationFn: async (reviewId: string) => {
      try {
        const result = await apiClient.post<SpinWheelResponse>(`/spin-win/spin-wheel/${reviewId}`)
    ...
    onError: (error) => {
      console.error("[v0] Spin wheel mutation error:", error)
    },
  })
}
